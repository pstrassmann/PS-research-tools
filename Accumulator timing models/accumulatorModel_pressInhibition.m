%% Accumulator model generator with press inhibition, July 2019, Patrick Strassmann
...
stimCenters = [0 1200 2100 3000];
stimWindowLength = 500;
...
for stimCond = 1:numel(stimCenters)
    ...
    if stimCenters(stimCond) ~= 0
         ...
         matStack(:,stimCenters(stimCond)-(stimWindowLength/2-1):stimCenters(stimCond)+(stimWindowLength/2-1)) = zeros(size(pressTimes_gen_filtered,1),stimWindowLength-1);

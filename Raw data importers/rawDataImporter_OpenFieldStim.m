...
% Tailored for importing data produced by Open Field stimulation apparatus
%
...
currWorkingDir = pwd;
numFilesSaved = 0;
try cd('/media/patrick_s/PS USB/Jin Lab USB/Hot Room/1 SingleLever/')
end
%-----------------------------------------
%**********************************************************
%**********************************************************
...
for particularFileInd=1:length(FileName)
    ...
    allCColonInds = strmatch('C:',dataCharMatrix,'exact'); %Looks for C:. Used to look for 0:. Data matrix must now be in matrix C (in MED-PC, DIM C= 50000...)
    allMColonInds = strmatch('M:',dataCharMatrix,'exact'); %Looks for K:
    rawDataStartMarkerInds = {allCColonInds, allMColonInds};
    ...
    for i=1:length(nameMatchLocations)
        ...
        if isequal(userName,strtrim(dataCharMatrix(nameMatchLocations(i),:)))
        for programLblIteration=1:IndOfA_AtEndOfprogramName-nameMatchLocations(i)-1
            ...
            programName = strtrim(dataCharMatrix(nameMatchLocations(i),:));
        end
        ...
        %Finds index of "C:" which is used to find start of dataMatrix 1 row after
        for iDataMatrix = 1:2
            allColonInds = rawDataStartMarkerInds{iDataMatrix};
            relevantColonInds = ((allColonInds>nameMatchLocations(i)).*allColonInds);
            relevantColonInds(~relevantColonInds)=NaN; %converts 0's to NaNs, so min can be computed
            dataStartInd = min(relevantColonInds)+2;
            ...
            while 1==1 %infinite loop until break
                ...
                if currentIndex>length(dataCharMatrix)
                    if iDataMatrix ==1; dataMatrixC = dataMatrix; else dataMatrixM = dataMatrix; end
                    break
        ...
        assignin('base', 'a', dataMatrixC);
        assignin('base', 'm', dataMatrixM);
        save(fileNameToSave,'a', 'm', 'subjectNumber', 'boxNumber', 'programName', 'startDate', 'startTime', 'endTime', 'experimentName')

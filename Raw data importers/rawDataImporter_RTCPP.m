...
% Tailored for importing data produced by Real-time place preference
% apparatus
...
for particularFileInd=1:length(FileName)
    ...
    allRColonInds = strmatch('R:',dataCharMatrix,'exact'); %Looks for K:
    rawDataStartMarkerInds = {allCColonInds, allRColonInds};
...
%         programName = [userName]; %userName assumed to be start of programName
        for programLblIteration=1:IndOfA_AtEndOfprogramName-nameMatchLocations(i)-1
            programName = [dataCharMatrix(nameMatchLocations(i),:) strtrim(dataCharMatrix(nameMatchLocations(i)+programLblIteration,:))];
        end
        ...
        for iDataMatrix = 1:2
            ...
            while 1==1 %infinite loop until break
                ...
                if currentIndex>length(dataCharMatrix)
                    if iDataMatrix ==1; dataMatrixC = dataMatrix; else dataMatrixR = dataMatrix; end
                    break
        ...
        assignin('base', 'r', dataMatrixR);
        save(fileNameToSave,'a', 'r', 'subjectNumber', 'boxNumber', 'programName', 'startDate', 'startTime', 'endTime', 'experimentName')

...
function [rvalMat,pValMat, fitY_cell] = plotCorr(x_cell,y_cell,wantPlots,color,subplotSpec)
...
fitY_cell = cell(size(x_cell,1),size(x_cell,2));
    ...
    for i = 1:size(x_cell,1)
        for j = 1:size(x_cell,2)
            ...
            if wantPlots ==1
                subplot(spX,spY,j)
                plot(x,y,'.','markersize',15,'markerfacecolor',color,'markeredgecolor','k')
                hold on; plot(x,fitY,'r','linewidth',3);
